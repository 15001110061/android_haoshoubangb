...
public class ScrollerNumberPicker extends View
{
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		Log.d("TALEX", "ScrollerNumberPicker---->onMeasure:controlWidth="
				+ controlWidth + "");
		if (controlWidth != 0)
		{
			// 重新定义View在界面上显示的大小
	...
	private void init(Context context, AttributeSet attrs)
	{
		TypedArray attribute = context.obtainStyledAttributes(attrs,
				R.styleable.ScrollerNumberPicker);
		unitHeight = (int) attribute.getDimension(
				R.styleable.ScrollerNumberPicker_unitHight, 32);
		normalFont = attribute.getDimension(
				R.styleable.ScrollerNumberPicker_normalTextSize, 14.0f);
		selectedFont = attribute.getDimension(
				R.styleable.ScrollerNumberPicker_selecredTextSize, 22.0f);
		itemNumber = attribute.getInt(
				R.styleable.ScrollerNumberPicker_itemNumber, 7);
		normalColor = attribute.getColor(
				R.styleable.ScrollerNumberPicker_normalTextColor, 0xff000000);
		selectedColor = attribute.getColor(
				R.styleable.ScrollerNumberPicker_selecredTextColor, 0xffff0000);
		lineColor = attribute.getColor(
				R.styleable.ScrollerNumberPicker_lineColor, 0xff000000);
		maskHight = attribute.getDimension(
				R.styleable.ScrollerNumberPicker_maskHight, 48.0f);
		noEmpty = attribute.getBoolean(
				R.styleable.ScrollerNumberPicker_noEmpty, false);
		isEnable = attribute.getBoolean(
				R.styleable.ScrollerNumberPicker_isEnable, true);
		attribute.recycle();
